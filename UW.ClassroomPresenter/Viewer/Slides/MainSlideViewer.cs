// $Id: MainSlideViewer.cs 1706 2008-08-12 22:39:28Z lamphare $
...
namespace UW.ClassroomPresenter.Viewer.Slides {
    public class MainSlideViewer : SlideViewer {
        ...
        void MainSlideViewer_MouseClick(object sender, MouseEventArgs e) {
            ...
            if (stylus is TextStylusModel || stylus is ImageStylusModel) {
                ...
                if (stylus is TextStylusModel) {
                    ...
                    //imagestylus
                    AddImageSheet(e.Location, scale_y, i_scale_x, i_scale_y);
                }
            }
        ...
        private void AddTextSheet(Point p, float i_scale_x, float i_scale_y) {
            ...
            using (Synchronizer.Lock(current_stylus_.SyncRoot)) {
                ...
                t = new TextSheetModel(Guid.NewGuid(), "", true, true, current_stylus_.DrawingAttributes.Color,
        ...
        /// <summary>
        /// Adds an image sheet at a point relative to the user's view
        /// </summary>
        /// <param name="p">Where we want the image sheet to pop up</param>
        /// <param name="i_scale_x"></param>
        /// <param name="i_scale_y"></param>
        private void AddImageSheet(Point p, float scale_y, float i_scale_x, float i_scale_y) {
            ImageSheetModel i;

            //get the image that the user wants to load
            OpenFileDialog file_dialog = new OpenFileDialog();
            file_dialog.Filter = "JPEG files (*.jpg)|*.jpg|BMP files (*.bmp|*.bmp";
            if (file_dialog.ShowDialog() == DialogResult.OK) {
                /// get our image to load
                ...
                ///check to see if the image is too large. If it's okay,
                ///then load the image into an imagesheetmodel and add the sheet
                if (width > 1600 || height > 1200) {
                    MessageBox.Show("Sorry, image file is too large");
                } else {
                    int h = (int)(this.SlideHeight - 20 * scale_y);
                    int w = h * this.SlideWidth / this.SlideHeight;
                    ...
                    ///create our image sheet, whose location is inversely transformed.
                    i = new ImageSheetModel(Guid.NewGuid(), image, true,
                        new Point((int)(Math.Min(p.X, SlideWidth - 10 - width) * i_scale_x), (int)(Math.Min(p.Y, SlideHeight - height) * i_scale_y)), new Size((int)(width * i_scale_x), (int)(height * i_scale_y)), 0);
                    ///add the sheet to our annotationsheets.
                    using (Synchronizer.Lock(this.Slide.SyncRoot)) {
                        this.Slide.AnnotationSheets.Add(i);
                    }
                }
            }
        }
